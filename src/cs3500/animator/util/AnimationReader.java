...
public class AnimationReader {
   ...
   */
  public static <AnimationModel> AnimationModel parseFile(
          Readable readable,
  ...
  private static <AnimationModel> void readRotation(Scanner s,
                                                    AnimationBuilder<AnimationModel> builder) {
    String[] fieldNames = new String[]{"initial time", "initial x-coordinate",
            "initial y-coordinate", "initial width", "initial height",
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException("Rotation: Expected a shape name, but no more input available");
    }
    for (int i = 0; i < 18; i++) {
      vals[i] = getInt(s, "Rotation", fieldNames[i]);
    }
    builder.addMotionRotation(name,
            vals[0], vals[1], vals[2], vals[3], vals[4], vals[5], vals[6], vals[7],
            vals[8], vals[9], vals[10], vals[11], vals[12], vals[13], vals[14], vals[15],
            vals[16],vals[17]);
  }
